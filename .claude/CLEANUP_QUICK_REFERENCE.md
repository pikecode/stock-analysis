# 数据清理快速参考

## 快速清理股票数据

### 最简单的方式（推荐）

```bash
./clean_stock_data.sh
```

就这么简单！脚本会：
1. ✅ 显示确认提示（防止误操作）
2. ✅ 清空 10 个股票相关表（~580,000 条记录）
3. ✅ 自动验证结果

### 直接使用 SQL（跳过确认）

```bash
psql -U peak -d stock_analysis -f clean_stock_data.sql
```

### 自定义用户名

```bash
DB_USER=your_username ./clean_stock_data.sh
```

---

## 清理的表（共 10 个）

| 表名 | 作用 | 删除记录数 |
|------|------|----------|
| stocks | 股票主数据 | 6,407 |
| concepts | 概念分类 | 563 |
| industries | 行业分类 | 110 |
| stock_concepts | 股票-概念关系 | 53,439 |
| stock_industries | 股票行业关系 | 5,426 |
| stock_metric_data_raw | 指标数据（分区） | 16,728 |
| concept_stock_daily_rank | 排名数据（分区） | 109,624 |
| concept_daily_summary | 汇总数据 | 1,124 |
| import_batches | 导入批次 | 14 |
| stock_concept_mapping_raw | CSV 导入原始数据 | 386,604 |
| **总计** | | **~579,000** |

---

## 典型工作流

### 导入测试循环

```bash
# 1. 清空旧数据
./clean_stock_data.sh

# 2. 启动开发服务器
# 终端 1
cd backend && python -m uvicorn app.main:app --host 127.0.0.1 --port 8000 --reload

# 终端 2
cd frontend && npm run dev

# 3. 导入测试数据
# 打开: http://127.0.0.1:5173/admin/import
# 按照 test-data/QUICK_START.md 操作

# 4. 进行测试和验证
# ... 测试你的功能 ...

# 5. 重复（返回第 1 步）
```

---

## 文件位置

```
根目录 (/Users/peak/work/pikecode/stock-analysis/)
├── clean_stock_data.sh        ← 运行这个文件（推荐）
├── clean_stock_data.sql       ← SQL 脚本（自动调用）
├── CLEAN_STOCK_DATA_README.md ← 详细文档
└── ...
```

---

## 执行时间

- **清空 ~579,000 条记录**: 5-10 秒
- **验证结果**: < 1 秒
- **总计**: < 15 秒

---

## 常见问题

**Q: 脚本要求确认是什么意思？**
A: 这是安全机制，防止误操作。按 `y` 确认，按 `n` 取消。

**Q: 执行出错了怎么办？**
A: 检查错误信息，常见原因：
- 用户名错误 → 用 `DB_USER=correct_user` 指定
- 数据库连接失败 → 确保 PostgreSQL 在运行
- 外键约束 → 说明有新表需要在脚本中处理

**Q: 能恢复被删除的数据吗？**
A: 无法恢复。应该定期备份：
```bash
pg_dump -U peak stock_analysis > backup_$(date +%Y%m%d).sql
```

---

## 相关文档

- 📖 **完整文档**: `CLEAN_STOCK_DATA_README.md`
- 🚀 **导入指南**: `test-data/QUICK_START.md`
- 📊 **测试数据**: `test-data/README.md`
- 🔄 **导入流程**: `.spec-workflow/data-import-workflow.md`

---

**创建时间**: 2025-11-28
**脚本版本**: 1.0
**提交**: 0f71bd4
