...
// 加载股票列表（初始化或远程搜索），只返回以1开头的转债
const loadStocks = async (keyword: string = '') => {
  stockLoading.value = true
  try {
    // 如果没有关键字，默认搜索 "1" 开头的股票
    // 如果有关键字，在关键字前加 "1" 确保只搜索转债
    const searchKeyword = keyword ? keyword : '1'

    const response = await stockApi.getList({
      keyword: searchKeyword.trim(),
      page_size: 50, // 转债数量较少，直接返回更多结果
    })
    // 过滤出以1开头的转债
    stockList.value = response.items
      .filter((item: any) => item.stock_code.startsWith('1'))
      .map((item: any) => ({
        stock_code: item.stock_code,
        stock_name: item.stock_name,
      }))
  } catch (error) {
    console.error('加载转债列表失败:', error)
    stockList.value = []
  } finally {
